...
bf_id = 2;
para.name = BF{2}; % Gamma functions
para.order = 10; % for Gamma functions or Fourier set
...
data_deconv = rsHRF_iterative_wiener_deconv(zdata,hrf,100);
...
figure;plot((1:length(hrfa(:,1)))*TR/para.T,hrfa(:,1),'b');xlabel('Time (s)')
title(['HRF (',BF{bf_id},')'])
figure;plot((1:nobs)*TR,zscore(data(:,1)));
