...
data_path = '/combinelab/03_user/younghyun/01_project/01_Signalflow_Hierarchy/data';    
switch module_type
    case '22modules'
        modules = importdata(fullfile(data_path,'/module.txt'));
        num_modules = 22;
    case '27modules'
...
EC = normalize_connectivity(EC);
EC = EC - eye(size(EC, 1));

% Compute the energy flow for the system
signal = linear_propagation(EC, z);
...
for current_roi = 1:num_modules
    ...
    % Compute the mean positive s ignal flow for the current ROI
    signal_flow_pos(current_roi) = mean(max(signal(end, seed), 0));

    % Compute the mean negative signal flow for the current ROI
    signal_flow_neg(current_roi) = mean(min(signal(end, seed), 0));
end