function FASK = run_fask(BOLD, oAlpha, threshold, iter)
...
parfor j = 1:iter
    temp= subsampleFASK(BOLD, j, oAlpha);
    FASKs = FASKs+temp;
end
...
delete BOLD* FASK* FAS* causal-cmd.log*
end

function temp = subsampleFASK(BOLD, j, oAlpha)
...
command = sprintf('java  -jar causal-cmd-1.12.0-jar-with-dependencies.jar --algorithm fask --test fisher-z-test --orientationAlpha %d --faskAdjacencyMethod 1 --alpha %d --data-type continuous --dataset BOLDsignal%d.txt --delimiter comma --no-header --prefix FASK%d --score sem-bic-score', oAlpha, 1e-6, j, j);
system(command)

% Convert the output to a connectivity matrix
temp = Tetrad2Matrix(sprintf('FASK%d_out.txt', j), 'directed','fask',0)';
