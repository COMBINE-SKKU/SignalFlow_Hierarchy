function [mat]=Tetrad2Matrix(tet,d,model,timelag) %,outputfile)
...
switch model
    ...
    case {'boss', 'lingam','grasp'}
        e{1}='Graph Attributes:'; 
        f=find(strncmp(tet,e,10))-2;
    case 'ccd'
        e{1}='Underline triples:';
        f=find(strncmp(tet,e,7))-4; 
end
...
if timelag
    ...
    % Loop through each line in the tet cell array
    for i = 1:length(tet)
        % Check if the pattern 'C*:1' occurs anywhere in the line
        if ~isempty(regexp(tet{i}, 'C\d+:1', 'once'))
            occurrenceCount = occurrenceCount + 1;
            if occurrenceCount == 2
                indexCStar1 = i;
                break; % Exit the loop once the second occurrence is found
            end
        end
    end
    fi1 = indexCStar1-1;
...
%      %node 2
    ...
    elseif strcmp(d,'pattern');
        ...
        if strcmp(at,'-') && strcmp(ah,'>')
            mat(n1,n2,1)=1;
        elseif strcmp(at,'o') && strcmp(ah,'o')
            mat(n1,n2,1)=1;
            mat(n2,n1,1)=1;
        end
...
if timelag
    ...
    %      %if the labels are not X# then use this
    %      %node 1
          n1=find(strcmp(R,regexprep(n{2},':1$',''))); % assign the position in the matrix of the corresponding AAL roi, ignoring ':1'
    %      %node 2
          n2=find(strcmp(R,regexprep(n{4},':1$',''))); % assign the position in the matrix of the corresponding AAL roi, ignoring ':1'
